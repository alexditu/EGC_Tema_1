...
DrawingWindow::DrawingWindow(int argc, char** argv,int _width, int _height, int startX, int startY, string name)
{
	...
	// Am modificat culoarea fundalului in negru
	//glClearColor(1,1,1,1);  //afiseaza fondul ferestrei aplicatiei in alb
	glClearColor(0,0,0,1);  //afiseaza fondul ferestrei aplicatiei in negru
...
void DrawingWindow::keyboardUpFunction(unsigned char key, int x, int y)
{
	onKeyUp(key);
}
...
void DrawingWindow::specialKeyUp(int key, int x, int y) {
	onSpecialKeyUp(key);
}
